%% Example of spherical distance-varying filters (DVF) and ear-centering
%
% Summary:
...
%   4) Translate the reference from head's center to ear's position (direct ear-centering).
%   5) Performs distance extrapolation on the sparse spherical set
%      from a original distance "a" to a new distance "b".
%   4) Translate the reference from ear's position to head's center (inverse ear-centering).
%
% Notation:
...
%% Direct ear-centering (from ear's position to head's center)
xHeadCenter = [0 0 0];
model = 'spherical';                            % Sound propagation model of translation operator.
direction = 'direct';                           % Direction of translation.
[~, DirectTranslatorLeft] = freeFieldTranslationOperator(x, xEarLeft, xHeadCenter, k, model, direction);
[~, DirectTranslatorRight] = freeFieldTranslationOperator(x, xEarRight, xHeadCenter, k, model, direction);
HLeft = DirectTranslatorLeft .* HLeft;
HRight = DirectTranslatorRight .* HRight;
...
%% Inverse ear-centering (from ear's position to head's center)
direction = 'inverse';                           % Direction of translation.
[~, InverseTranslatorLefta] = freeFieldTranslationOperator(aCart, xEarLeft, xHeadCenter, k, model, direction);
[~, InverseTranslatorRighta] = freeFieldTranslationOperator(aCart, xEarRight, xHeadCenter, k, model, direction);
[~, InverseTranslatorLeftb] = freeFieldTranslationOperator(bCart, xEarLeft, xHeadCenter, k, model, direction);
[~, InverseTranslatorRightb] = freeFieldTranslationOperator(bCart, xEarRight, xHeadCenter, k, model, direction);
HLefta = InverseTranslatorLefta .* HLefta;
HRighta = InverseTranslatorRighta .* HRighta;
HLeftb = InverseTranslatorLeftb .* HLeftb;
HRightb = InverseTranslatorRightb .* HRightb;
